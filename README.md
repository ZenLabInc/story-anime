...
> 実験的なプロトタイプです。アプリの月額利用料はありませんが、会話・画像生成にはGoogleのAPI利用料金がかかります。「生成して」「どんどん進めて」と依頼すると、その範囲で画像生成が進みます。正面画像にOKを出すと横・背面をまとめて生成します。
...
ログインや表示名の登録は不要です。設定画面ではAPIキーと、会話・画像生成に使用した入力／出力トークン数を確認できます。停止はターミナルで **Ctrl+C**。次回は同じフォルダで上の起動コマンドを実行するだけです。

## 4. 自分のAPIキーを登録する

1. [Google AI Studio](https://aistudio.google.com/apikey) でGemini APIキーを取得します。
2. Google側で画像生成モデルが利用できることと、請求設定・料金・利用上限を確認します。
3. アプリ左下の設定を開き、「Gemini APIキー」に入力して保存します。
...
| パス | 役割 |
|---|---|
| `app/` | HTTPサーバー、会話エージェント、制作・保存処理 |
| `web/` | チャット画面とローカル設定画面 |
| `planning/` | モデルと費用計算の前提 |
| `scripts/` | バックアップ、復元、費用・指標確認 |
| `tests/` | 外部APIを呼ばない自動テスト |
| `docs/` | 製品仕様、データの扱い、検証記録 |
